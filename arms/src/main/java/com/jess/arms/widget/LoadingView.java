...
public class LoadingView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mSize = (int) ta.getDimension(R.styleable.LoadingView_lv_size, ArmsUtils.dip2px(context, 100));
        setSize(mSize);

        mColor = ta.getColor(R.styleable.LoadingView_lv_color, Color.parseColor("#333333"));
        setColor(mColor);

        mDuration = ta.getInt(R.styleable.LoadingView_lv_duration, 1800);

        mPadding = (int) ta.getDimension(R.styleable.LoadingView_lv_padding, ArmsUtils.dip2px(context, 6));
    ...
    protected void onDraw(Canvas canvas) {
        if (mSize > 0 && mPadding > 0) {
            int size = mSize - mPadding;
